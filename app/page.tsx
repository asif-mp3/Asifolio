...
const BackgroundAnimation = () => {
  ...
  useEffect(() => {
    const generateDots = (count: number, offset: number, sizeRange: [number, number], range: number, lightness: number) =>
      [...Array(count)].map((_, i) => ({
        id: i + offset,
        x: Math.random() * 100,
        y: Math.random() * 100,
        size: Math.random() * (sizeRange[1] - sizeRange[0]) + sizeRange[0],
        color: `hsl(${Math.random() * 60 + 200}, 100%, ${lightness}%)`,
        range,
        opacity: 0.1 + Math.random() * 0.5,
      }))

    setDots([...generateDots(20, 0, [1, 4], 2, 70), ...generateDots(10, 20, [2, 5], 10, 75)])
  }, [])
